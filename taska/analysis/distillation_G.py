"""G-distillation control: same as distillation.py but with G as teacher.

If G gives the same ~30% speedup as M-distillation, the speedup is generic
soft-target effect (Hinton-style dark knowledge), nothing M-specific.

If M-distillation >> G-distillation, M has unique useful structure beyond
what G has.

Usage:
    python taska/analysis/distillation_G.py
"""
import sys
...
def run_config(lam, device, teacher_state):
    ...
    teacher.eval()
    with t.no_grad():
        teacher_logits_train = teacher(train_in)[:, -1, :]
...
def main():
    ...
    teacher_state = load_state(HERE / 'checkpoints' / 'G' / 'final.pt')

    histories = {}
    for lam in LAMBDAS:
        histories[lam] = run_config(lam, device, teacher_state)

    out_json = HERE / 'results' / 'distillation_G.json'
    ...
    for color, lam in zip(cmap, LAMBDAS):
        h = histories[lam]
        ax.plot(h['epoch'], h['test_acc'], label=f'λ={lam}', color=color)
    ax.set_xlabel('epoch'); ax.set_ylabel('test accuracy')
    ax.set_title('G-distillation control: student trained with G as teacher')
    ax.legend(); ax.grid(True, alpha=0.3); ax.set_ylim(-0.05, 1.05)
    out = HERE / 'results' / 'fig_distillation_G.png'
